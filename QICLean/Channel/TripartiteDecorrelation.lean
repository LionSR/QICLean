...
This file develops the generic finite-dimensional quantum-information core for
decorrelation on an explicit tensor product \(H_A\otimes H_X\otimes H_B\).
It includes tensor lifts, marginals and support projectors, matrix-unit
expansions, the observable and complex-linear decorrelation conditions,
support absorption, and the resulting support-product identities.

## Main definitions

* `TripartiteDecorrelation.IsObservableDecorrelated`: decorrelation tested on
  Hermitian observables.
* `TripartiteDecorrelation.IsDecorrelated`: its complex-linear extension.
* `TripartiteDecorrelation.supportAX` and
  `TripartiteDecorrelation.supportXB`: the two marginal support projectors.
* `TripartiteDecorrelation.liftAX` and `TripartiteDecorrelation.liftXB`: local
  lifts to the tripartite space.

## Main results

* `TripartiteDecorrelation.isObservableDecorrelated_iff`
* `TripartiteDecorrelation.IsDecorrelated.supports_mul_complement_mul_eq_zero`
* `TripartiteDecorrelation.liftSupportAX_mul_self` and
  `TripartiteDecorrelation.liftSupportXB_mul_self`, with their right-handed
  forms.
* `TripartiteDecorrelation.supportProducts_eq`
* `TripartiteDecorrelation.range_mul_eq_inter`
...
theorem liftA_comm_liftXB (OA : Matrix A A ℂ) (M : Matrix (X × B) (X × B) ℂ) :
...
theorem liftB_comm_liftAX (OB : Matrix B B ℂ) (M : Matrix (A × X) (A × X) ℂ) :
