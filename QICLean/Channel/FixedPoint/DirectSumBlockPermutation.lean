...
theorem exists_blockEquiv_dim_eq_of_starAlgEquiv_pi_matrix
    ...
    ∃ σ : ι ≃ κ,
      (∀ i, T.toRingEquiv.mapTwoSidedIdeal
        (TwoSidedIdeal.blockIdeal (fun k ↦ Matrix (Fin (d k)) (Fin (d k)) ℂ) i) =
          TwoSidedIdeal.blockIdeal (fun k ↦ Matrix (Fin (e k)) (Fin (e k)) ℂ) (σ i)) ∧
      ∀ i, d i = e (σ i) := by
  classical
  obtain ⟨σ, hσ⟩ :=
    TwoSidedIdeal.exists_blockEquiv_of_ringEquiv_pi_simple T.toRingEquiv
  refine ⟨σ, hσ, ?_⟩
  intro i
  let φ := blockComponentLinearMap T σ i
  have hφBij : Function.Bijective φ := by
    exact TwoSidedIdeal.blockComponentMap_bijective T.toRingEquiv σ hσ i
  ...
  have hSingle (i : ι) (M : Matrix (Fin (d i)) (Fin (d i)) ℂ) :
    ...
    · intro j _ hj
      have hzero : T (Pi.single i M) j = 0 := by
        exact TwoSidedIdeal.ringEquiv_maps_single_support_between
