...
theorem IsIndecomposablePositiveMap.exists_isPPT_not_isSeparable
  ...
  have hW₀nonneg : ∀ ψ : Fin d × Fin d' → ℂ, HasSchmidtRankLE 1 ψ →
    ...
    intro ψ hψ
    have hquad := ChoiJamiolkowski.IsNPositiveMap.choiMatrix_quadraticForm_nonneg_of_hasSchmidtRankLE_rectangular
  ...
  have hWnot : ¬ IsDecomposableWitness W := by
    intro hWdec
    apply hT.2
    apply (ChoiRectangular.isDecomposablePositiveMap_iff_choiMatrix_traceAdjointMap_isDecomposableWitness
      T).mpr
