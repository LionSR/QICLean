...
`Matrix (Fin D) (Fin D) ℂ` generated by a finite family of matrices.

The main result is `Matrix.burnside_matrix`:
if a family `A : Fin d → Matrix (Fin D) (Fin D) ℂ` acts irreducibly on `ℂ^D` (no
nontrivial invariant submodule), then the unital subalgebra it generates is all of `M_D(ℂ)`.
...
If the matrices in `A : Fin d → Matrix (Fin D) (Fin D) ℂ` act irreducibly on `ℂ^D`
(in the sense `IsIrreducibleAction`), then the unital subalgebra they generate is all of `M_D(ℂ)`.
...
theorem burnside_matrix [NeZero D]
    (A : Fin d → Matrix (Fin D) (Fin D) ℂ) (hIrr : IsIrreducibleAction (d := d) (D := D) A) :
