...
This file contains the algebraic core of the finite-dimensional specialization
of Pedro Nowosad's Theorem 1.8.  The first- and second-variation identities
are packaged by `MinimumData`.  Comparing the two nonnegative quadratic
forms gives the power recursion corresponding to equations (1.19)--(1.20),
the Laurent-polynomial identity (1.23), and the product rule (1.28).

For the finite coordinate algebra, the Laurent algebra consists precisely of
the functions constant on the value classes of the generator.  Its explicitly
named Lagrange idempotents therefore kill every coordinate point derivation.
This is the finite-dimensional form of the final Singer--Wermer step.
...
theorem valueClassIdempotent_mem_polynomialSubalgebra [Finite ι]
    (w : ι → ℝ) (i : ι) :
...
algebra. -/
theorem valueClassIdempotent_mem_laurentSubalgebra [Finite ι]
    (w : ι → ℝ) (i : ι) :
...
theorem inv_mem_polynomialSubalgebra [Finite ι] (w : ι → ℝ) :
...
theorem laurentSubalgebra_eq_polynomialSubalgebra [Finite ι] (w : ι → ℝ) :
...
omit [Fintype ι] in
/-- Conversely, every function which is constant on the value classes of
`w` belongs to the finite Laurent algebra generated by `w`.  The proof is
the explicit interpolation sum over the distinct values of `w`. -/
theorem mem_laurentSubalgebra_of_eq_on_valueClass [Finite ι]
    {w q : ι → ℝ} (hq : ∀ i j, w i = w j → q i = q j) :
    q ∈ laurentSubalgebra w := by
  classical
  let _ := Fintype.ofFinite ι
  let values : Finset ℝ := Finset.univ.image w
  let representative : ↥values → ι := fun r ↦
    (Finset.mem_image.mp r.property).choose
  have hrepresentative (r : ↥values) : w (representative r) = r :=
    (Finset.mem_image.mp r.property).choose_spec.2
  let interpolation : ι → ℝ :=
    ∑ r : ↥values, q (representative r) •
      valueClassIdempotent w (representative r)
  have hinterpolation_mem : interpolation ∈ laurentSubalgebra w := by
    dsimp [interpolation]
    apply Subalgebra.sum_mem
    intro r _
    exact (laurentSubalgebra w).smul_mem
      (valueClassIdempotent_mem_laurentSubalgebra w (representative r)) _
  have hinterpolation_eq : interpolation = q := by
    funext j
    dsimp [interpolation]
    simp only [Finset.sum_apply, Pi.smul_apply, smul_eq_mul]
    let rj : ↥values := ⟨w j, Finset.mem_image.mpr
      ⟨j, Finset.mem_univ j, rfl⟩⟩
    rw [Finset.sum_eq_single rj]
    · have hsame : w j = w (representative rj) := by
        simpa [rj] using (hrepresentative rj).symm
      rw [valueClassIdempotent_apply]
      simp only [hsame, ↓reduceIte, mul_one]
      exact hq _ _ hsame.symm
    · intro r _ hr
      have hne : w j ≠ w (representative r) := by
        intro hvalue
        apply hr
...
theorem coordinate_pointDerivation_eq_zero [Finite ι]
