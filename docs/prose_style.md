...
This document specifies the prose conventions for **all reader-facing text in QICLean**.
...
- In migrated docstring regions (none currently designated in QICLean; this
  was `TNLean/MPS/ParentHamiltonian` in the monorepo this library was
  extracted from), inline mathematical expressions should use `\(...\)`, not
  backtick code spans. Reserve backticks for genuine Lean references.
