...
\begin{proof}
    \leanok
    A nonzero vector space over a division ring is a simple module over its
    ring of endomorphisms. The algebra isomorphism
    $M_n(K)\cong\operatorname{End}_K(K^n)$ sending $A$ to $v\mapsto Av$
    transports that simplicity: the identity of $K^n$ is semilinear along the
    isomorphism, since the isomorphism carries the matrix action to the
    action by endomorphisms, and a bijective semilinear map along a surjective
    ring homomorphism carries simplicity of the target module to the source.
    Commutativity of $K$ enters only through the isomorphism, the matrix
    action on $K^n$ being $K$-linear only for commutative $K$.
\end{proof}
