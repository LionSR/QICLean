...
\begin{definition}[Adjoint dissipator]
    ...
    \begin{align}
        ...
        \label{eq:semigroup_adjoint_dissipator}
    \end{align}
\end{definition}
...
\begin{definition}[Adjoint generator]
    ...
    \begin{align}
        L^*(A)
        &=i[H,A]+\sum_j\mathcal{D}_{L_j}^*(A).
        \label{eq:semigroup_adjoint_generator}
    \end{align}
\end{definition}
...
\begin{theorem}[Trace-pairing characterization of the adjoint generator]
    ...
    \begin{align}
        \tr(\rho L^*(A))
        &=\tr(L(\rho)A).
        \label{eq:semigroup_trace_pairing}
    \end{align}
\end{theorem}
...
\begin{theorem}[Adjoint kernel lies in the commutant]
    ...
    (\ref{eq:semigroup_trace_pairing}).
    If $L$ has a faithful stationary state $\rho_0>0$ with $L(\rho_0)=0$,
    then, for any $A$, the condition $L^*(A)=0$ implies, for every $j$,
    \begin{align}
        [A,H]=[A,L_j]=[A,L_j^\dagger]
        &=0.
        \label{eq:semigroup_kernel_commutators}
    \end{align}
...
\begin{proof}
    ...
    Faithfulness of $\rho_0$ and
    $\tr(\rho_0L^*(A^\dagger A))=0$
...
\begin{theorem}[Adjoint kernel equals the commutant]
    ...
    \begin{align}
        \ker(L^*)
        &=\{H,L_j,L_j^\dagger\}'.
        \label{eq:semigroup_kernel_commutant}
    \end{align}
    That is, the adjoint kernel equals the commutant of
    $\{H,L_j,L_j^\dagger\}$.
    This is \cite[Theorem~7.2]{Wolf2012Quantum}.
\end{theorem}
...
\begin{proof}
    ...
    The reverse inclusion $\subseteq$ is
    Theorem~\ref{thm:adjointKernel_eq_commutant}.
\end{proof}
...
\begin{definition}[Rank-deficient fixed density]\label{def:has_rank_deficient_fixed_density}
    ...
    kernel such that $e^{tL}(\rho_0)=\rho_0$ for all $t\ge0$.
\end{definition}
...
\begin{definition}[Invariant compression]\label{def:has_invariant_compression}
    ...
    that $e^{tL}(P\MN{D}P)\subseteq P\MN{D}P$ for all $t\ge0$.
\end{definition}
...
\begin{definition}[Block-upper-triangular Lindblad form]
    ...
    $(H,\{L_j\})$ for $L$ such that $(\Id-P)L_jP=0$ and
    $(\Id-P)\kappa P=0$ for all $j$, where
...
\begin{definition}[Generator-preserved compression]
    ...
    For a projection $P$, the generator preserves the compression
    $P\MN{D}P$ if, for every $X\in\MN{D}$,
    \begin{align}
        P L(PXP)P
        &=L(PXP).
        \label{eq:semigroup_generator_compression}
    \end{align}
\end{definition}
...
\begin{theorem}[Semigroup invariance implies generator invariance]
    ...
    If the semigroup preserves $P\MN{D}P$ for all non-negative times, then the generator
...
\begin{theorem}[Generator invariance implies semigroup invariance]
    ...
    \uses{def:generator_preserves_compression_ch12, def:has_invariant_compression}
    If the generator preserves $P\MN{D}P$, then so does the exponential semigroup
    $e^{tL}$ for every $t\ge0$.
\end{theorem}
...
\begin{theorem}[Rank-deficient fixed densities and kernel elements]
    ...
    $t\ge0$ if and only if it lies in $\ker L$.
    This is \cite[Proposition~7.6, (1)$\Leftrightarrow$(2)]{Wolf2012Quantum}.
\end{theorem}

\begin{proof}\leanok
    The equivalence $L(\rho)=0\iff e^{tL}(\rho)=\rho$ for all $t\ge0$
    follows from the generator--semigroup correspondence: exponentiate for
    the forward direction, and differentiate at $t=0$ for the reverse.
\end{proof}
...
\begin{proof}
    ...
    Theorem~\ref{thm:generator_preserves_compression_from_semigroup_ch12}.
    The generator-level vanishing condition forces $(\Id-P)L_jP=0$ and
    $(\Id-P)\kappa P=0$ through the sum-of-squares identity.
\end{proof}
...
\begin{theorem}[Triangular Lindblad data give deficient kernels]
    ...
    \lean{hasRankDeficientKernelElement_of_hasBlockUpperTriangularLindblad,
        IsChannel.exists_fixed_density_of_preserves_compression}\leanok
...
\begin{proof}
    ...
    compression $P\MN{D}P$.
    Therefore the semigroup $e^{tL}$ also preserves $P\MN{D}P$
    for all $t\ge0$.  Choose $t_0>0$ so that the fixed-point space of
    $e^{t_0L}$ is $\ker L$.  The invariant compression contains a density
    matrix $\rho_0$ fixed by $e^{t_0L}$, obtained from the Cesàro means of
    any density matrix supported in $P\MN{D}P$.  Hence
    $L(\rho_0)=0$ and $P\rho_0P=\rho_0$.  Since $P\neq\Id$, the matrix
    $\rho_0$ has nontrivial kernel.
\end{proof}
...
\begin{proof}\leanok
    ...
    The conclusion records the adjacent equivalences
    (1)$\leftrightarrow$(2), (2)$\leftrightarrow$(3), and
    (3)$\leftrightarrow$(4).  They follow from
    (1)$\leftrightarrow$(2), (1)$\to$(3),
    (3)$\leftrightarrow$(4), and (4)$\to$(2).
\end{proof}
...
\begin{definition}[Hermitian-closed Lindblad span]\label{def:is_lindblad_span_hermitian_closed}
    ...
    i.e.\ it forms a $*$-subspace of $\MN{D}$.
\end{definition}
...
\begin{definition}[Trivial Lindblad commutant]\label{def:has_lindblad_span_trivial_commutant}
    ...
    i.e.\ the Lindblad operators act irreducibly on $\MN{D}$.
\end{definition}
...
\begin{theorem}[$\kappa$ block vanishing from generator compression]%
    ...
    $\kappa=iH+\tfrac{1}{2}\sum_jL_j^\dagger L_j$, and let $P$ be an
    orthogonal projection such that the generator preserves the compression
    $P\MN{D}P$.
    If $(\Id-P)L_jP=0$ for every $j$, then $(\Id-P)\kappa P=0$.
\end{theorem}

\begin{proof}\leanok
    \uses{def:lindblad_form}
    From $L=\varphi-\kappa(\cdot)-(\cdot)\kappa^\dagger$ and
    $(\Id-P)L(P)=0$: the $(\Id-P)\varphi(P)$ term vanishes because each
    $(\Id-P)L_jP=0$, and the $(\Id-P)(P\kappa^\dagger)$ term vanishes
    because $(\Id-P)P=0$. What remains is $(\Id-P)\kappa P=0$.
\end{proof}
...
\begin{theorem}[Full algebra generation forbids block-upper-triangular form]%
    ...
    matrix algebra $\MN{D}$, then the Lindblad data do not admit a
...
\begin{proof}\leanok
    ...
    $(\Id-P)AP=0$ for every generator. Since the generated algebra is
    all of $\MN{D}$, every matrix satisfies $(\Id-P)AP=0$, forcing
...
\begin{proof}\leanok
    ...
    $(\Id-P)L_jP=0$ and Hermitian closure give $PL_j(\Id-P)=0$ as
...
\begin{theorem}[Full algebra generation implies non-reducibility]
    ...
    If the algebra generated by $\{L_j\}$ and $\kappa$ is the entire
    matrix algebra $\MN{D}$, then the QDS is not reducible.
