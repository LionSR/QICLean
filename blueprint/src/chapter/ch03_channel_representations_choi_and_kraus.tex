...
Section~\ref{sec:choi_jamiolkowski} established the maximally entangled state, the
...
The Choi foundations were established in Section~\ref{sec:choi_jamiolkowski}. The
...
$T(\rho)=\sum_i K_i\rho K_i^\dagger$. In index form,
$T(\rho)_{ab}=\sum_{i,c,d}(K_i)_{ac}\rho_{cd}\overline{(K_i)_{bd}}$.
% A tensor-network diagram for T(\rho)=\sum_i K_i\rho K_i^\dagger accompanied
% this formula in TNLean (Wolf, Theorem 2.1, Equation (2.8)). QICLean does
% not carry the tenkz diagram pipeline; the equations above are the complete
