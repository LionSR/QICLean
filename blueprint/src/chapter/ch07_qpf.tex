...
irreducibility criterion modeled on Wolf's spectral characterization appear
...
\input{chapter/ch07_qpf_cesaro_fixed_points}

\input{chapter/ch07_qpf_weighted_cesaro}
...
\begin{definition}[Support projection]\label{def:support_proj}
    ...
    $\rho=U\diag(\lambda_1,\ldots,\lambda_D)U^\dagger$, it is
    \begin{align}
        P
         & =
        U\diag\!\bigl(        \mathbf{1}_{\lambda_1>0},\ldots,
        \mathbf{1}_{\lambda_D>0}
        \bigr)U^\dagger,
        \label{eq:qpf_support_projection}
    \end{align}
    where $\mathbf{1}_{\lambda_j>0}$ is $1$ if $\lambda_j>0$ and $0$ otherwise.
\end{definition}

\begin{theorem}[Vanishing complementary Kraus corners]
    \label{thm:kraus_fixed_support_lower_zero}
    \lean{Kraus.lowerZero_of_posSemidef_fixedPoint}
    \leanok
    \uses{def:kraus_map, def:support_proj}
    Let $\rho\geq0$ be fixed by the Kraus map of a finite matrix family $K$, and
    let $Q$ be the support projection of $\rho$. Then $Q$ is an orthogonal
    projection and
    \begin{align}
        (\Id-Q)K_iQ
         & =0
...
\begin{proof}
    \leanok
    The matrices $Q$ and $\rho$ have the same kernel. For a vector in the
    orthogonal complement of the range of $Q$, the fixed-point identity writes
    its zero quadratic form against $\rho$ as a sum of nonnegative quadratic
    forms. Each summand vanishes, so $QK_i^\dagger(\Id-Q)=0$. Taking adjoints
    gives $(\Id-Q)K_iQ=0$.
\end{proof}

\begin{theorem}[Vanishing lower corners imply invariant compression]
    \label{thm:kraus_lower_zero_invariant_compression}
    \lean{Kraus.lowerZero_implies_invariance}
    \leanok
    \uses{def:kraus_map}
    Let $K$ be a finite matrix family with Kraus map $E$, and let $Q$ be an
    orthogonal projection. If $(\Id-Q)K_iQ=0$ for every $i$, then
    \begin{align}
        Q E(QXQ)Q
         & =E(QXQ)
        \notag
...
\begin{proof}
    \leanok
    The lower-corner identity gives $K_iQ=QK_iQ$; taking adjoints gives
    $QK_i^\dagger=QK_i^\dagger Q$. Substituting these two equalities in every
    summand of the Kraus representation of $E(QXQ)$ proves the compression
    identity.
\end{proof}
...
\begin{theorem}[Invariant support compression for a CP fixed point]
    ...
    \leanok
    \uses{def:cp_map, def:support_proj}
...
\begin{proof}
    \leanok
    \uses{thm:kraus_fixed_support_lower_zero,
        thm:kraus_lower_zero_invariant_compression}
...
\begin{proof}
    \leanok
    \uses{thm:transfer_cp, thm:psd_fp_pd_irred}
    The transfer map is completely positive, so
    Theorem~\ref{thm:psd_fp_pd_irred} applies.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:irred_map_smul, thm:psd_fp_unique_irred_cp}
    Choose Kraus operators for $E$ and rescale each by $r^{-1/2}$. Their
    transfer map is $r^{-1}E$, so both $\rho$ and $\sigma$ are fixed points.
    The rescaled map remains irreducible and completely positive, and
    Theorem~\ref{thm:psd_fp_unique_irred_cp} gives the claimed
    proportionality.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:psd_fp_exists, thm:transfer_psd_fp_pd_irred,
...
\input{chapter/ch07_qpf_irreducibility_and_ergodicity}
