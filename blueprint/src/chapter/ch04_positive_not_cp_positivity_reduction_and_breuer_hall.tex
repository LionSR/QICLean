...
For a positive semidefinite matrix $P$, Wolf introduces the cone $C(P)$ in the
proof of the implication (1) $\Rightarrow$ (2) of
\cite[Proposition~3.6]{Wolf2012Quantum}. Equation~\eqref{eq:psd_cone_face}
is the definition of $C(P)$ below; it is not one of the standard-form equations
for a positive map.
The next results identify its linear span and compute its dimension. They
supply the cone-dimension step used in the formalization of the full
equivalence in Proposition~3.6 below.

\begin{definition}[Wolf's cone $C(P)$]
    \label{def:psd_cone_face}
    \lean{Matrix.psdConeFace}
    \leanok
    For $P\in\MN{D}$, define
    \begin{align}
        C(P)
         & :=\{A\in\MN{D}\mid \text{there is a real }c>0
             \text{ such that }P\ge cA\ge0\}.
        \tag{3.42}\label{eq:psd_cone_face}
    \end{align}
\end{definition}

\begin{lemma}[Support-corner characterization of $C(P)$]
    \label{lem:psd_cone_face_mem_iff}
    \lean{Matrix.mem_psdConeFace_iff}
    \leanok
    \uses{def:psd_cone_face}
    Let $P,A\in\MN{D}$ with $P\ge0$, and let $Q=\supp(P)$ be the support
    projection of $P$. Then
    \begin{align}
        A\in C(P)
         & \iff A\ge0\ \text{ and }\ QAQ=A.
        \notag
    \end{align}
\end{lemma}
...
\begin{proof}\leanok
    \uses{def:psd_cone_face}
    If $P\ge cA\ge0$ with $c>0$, then $A\ge0$ and every vector in the kernel of
    $P$ lies in the kernel of $A$. Hence $A$ is supported on $Q$, so $QAQ=A$.
    Conversely, suppose $A\ge0$ and $QAQ=A$. On the support of $P$, put
    $q=\lVert P^{-1/2}AP^{-1/2}\rVert$, where the inverse square root is taken
    only on that support, and choose $c=(q+1)^{-1}$. Then $c>0$ and
    $cP^{-1/2}AP^{-1/2}\le\Id$ on the support of $P$. Conjugating by the support
    square root gives $cA\le P$, and therefore $A\in C(P)$.
\end{proof}

\begin{theorem}[Span of Wolf's cone $C(P)$]
    \label{thm:psd_cone_face_span}
    \lean{Matrix.span_psdConeFace_eq_cornerSubmodule_supportProj}
    \leanok
    \uses{def:psd_cone_face, lem:psd_cone_face_mem_iff}
    Let $P\in\MN{D}$ be positive semidefinite and let $Q=\supp(P)$. Then the
    complex linear span of $C(P)$ is the full support corner:
    \begin{align}
        \operatorname{span}_{\C} C(P)
         & =Q\MN{D}Q
          =\{A\in\MN{D}\mid QAQ=A\}.
        \notag
    \end{align}
...
\begin{proof}\leanok
    \uses{lem:psd_cone_face_mem_iff}
    The support-corner characterization gives the inclusion from left to right.
    For the reverse inclusion, use that the positive semidefinite matrices span
    $\MN{D}$ over $\C$. Compress such a spanning expression by $Q$. Every
    compressed positive semidefinite matrix $QBQ$ is positive semidefinite and
    supported on $Q$, hence belongs to $C(P)$ by
    Lemma~\ref{lem:psd_cone_face_mem_iff}. Every matrix in the corner is equal
    to its compression, which proves the reverse inclusion.
\end{proof}

\begin{corollary}[Dimension of the cone span]
    \label{cor:psd_cone_face_span_dimension}
    \lean{Matrix.finrank_span_psdConeFace_eq_rank_sq}
    \leanok
    \uses{thm:psd_cone_face_span}
    If $P\in\MN{D}$ is positive semidefinite, then
    \begin{align}
        \dim_{\C}\!\left(\operatorname{span}_{\C} C(P)\right)
         & =\rank(P)^2.
        \notag
    \end{align}
\end{corollary}
...
\begin{proof}\leanok
    \uses{thm:psd_cone_face_span}
    By Theorem~\ref{thm:psd_cone_face_span}, the span is the matrix corner
    supported on $Q=\supp(P)$. This corner is linearly equivalent to
    $\MN{r}$, where $r=\rank(Q)$. The support projection has
    $\rank(Q)=\rank(P)$, equivalently $\tr(Q)=\rank(P)$, so the corner has
    complex dimension $r^2=\rank(P)^2$.
\end{proof}

\begin{theorem}[Positive Hermitian-rank preservers]
    \label{thm:positive_hermitian_rank_preserver_standard_form}
    \lean{IsPositiveMap.exists_isUnit_det_conj_or_transpose_of_preserves_hermitian_rank}
    \leanok
    \uses{def:positive_map, thm:conjugation_filters_preserve_positivity,
        lem:conj_inv_sqrt_normalizes,
        thm:hermitian_spectrum_preserver_classification}
    Let $T:\MN{D}\to\MN{D}$ be a positive complex-linear map. Suppose that
    \begin{align}
        \rank(T(H)) & =\rank(H)
        \notag
    \end{align}
    for every Hermitian matrix $H\in\MN{D}$. Then there is an invertible matrix
    $Y\in\MN{D}$ such that either
    \begin{align}
        T(X) & =YXY^\dagger
        \notag
    \end{align}
    ...
    \begin{align}
        T(X) & =YX^{\mathsf T}Y^\dagger
        \notag
    \end{align}
    for every $X\in\MN{D}$. This is precisely the implication
    (2) $\Rightarrow$ (3) of \cite[Proposition~3.6]{Wolf2012Quantum}.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:conjugation_filters_preserve_positivity,
        lem:conj_inv_sqrt_normalizes,
        thm:hermitian_spectrum_preserver_classification}
    Put $A=T(\Id)$, $S=\sqrt{A}$, and $R=S^{-1}$. Positivity and rank
    preservation imply that $A$ is positive definite. The normalized map
    \begin{align}
        \widetilde T(X) & =R\,T(X)\,R
        \notag
    \end{align}
    is therefore positive, unital, and rank preserving on Hermitian matrices.
    For every Hermitian $H$ and every real $\lambda$, unitality and the
    rank-drop characterization of the spectrum give
    \begin{align}
        \lambda\in\sigma(\widetilde T(H))
        & \iff \rank\!\left(\widetilde T(H)-\lambda\Id\right)<D \\
        & \iff \rank\!\left(\widetilde T(H-\lambda\Id)\right)<D \\
        & \iff \rank(H-\lambda\Id)<D
        \iff \lambda\in\sigma(H).
        \notag
    \end{align}
    Thus $\widetilde T$ preserves the spectrum of every Hermitian matrix.
    Theorem~\ref{thm:hermitian_spectrum_preserver_classification} yields a
    unitary $U$ such that $\widetilde T(X)=UXU^\dagger$ for all $X$, or
    $\widetilde T(X)=UX^{\mathsf T}U^\dagger$ for all $X$. Since
    $T(X)=S\widetilde T(X)S$, either formula has the required form with the
    corrected denormalization factor
    \begin{align}
        Y & =\sqrt{T(\Id)}\,U.
        \notag
    \end{align}
    This factor, rather than $U(T(\Id))^{-1/2}$ printed in the final line of
    the cited proof, undoes the displayed normalization.
\end{proof}
...
\begin{theorem}[Automorphisms and rank-preserving maps]
    ...
    \begin{enumerate}
        ...
        \item there is an invertible $Y\in\MN{D}$ such that either
              $T(X)=YXY^\dagger$ for every $X$, or
              $T(X)=YX^{\mathsf T}Y^\dagger$ for every $X$.
    \end{enumerate}
    This is \cite[Proposition~3.6]{Wolf2012Quantum}.
\end{theorem}

\begin{proof}\leanok
    \uses{def:psd_cone_face, cor:psd_cone_face_span_dimension,
        thm:positive_hermitian_rank_preserver_standard_form,
        thm:conjugation_maps_psd_cone_onto}
    For (1) $\Rightarrow$ (2), $T$ is bijective and $T^{-1}$ is again positive
    and maps the positive semidefinite cone onto itself. For $P\geq0$, $T$ maps
    Wolf's cone $C(P)$ onto $C(T(P))$. The complex spans therefore have equal
    dimension, so
    \begin{align}
        \rank(P)^2
        &=\dim_{\C}\!\left(\operatorname{span}_{\C}C(P)\right)\\
        &=\dim_{\C}\!\left(\operatorname{span}_{\C}C(T(P))\right)
        =\rank(T(P))^2.
        \notag
    \end{align}
    Hence $T$ preserves rank on positive semidefinite matrices. Decompose a
    Hermitian $H$ as $H=P_+-P_-$ into its positive and negative parts. Rank
    subadditivity gives $\rank(T(H))\leq\rank(H)$; applying the same argument to
    $T^{-1}$ gives the converse inequality. Implication (2) $\Rightarrow$ (3)
    is Theorem~\ref{thm:positive_hermitian_rank_preserver_standard_form}, and
    (3) $\Rightarrow$ (1) is
    Theorem~\ref{thm:conjugation_maps_psd_cone_onto}.
\end{proof}
...
\begin{theorem}[The Breuer--Hall map is indecomposable]
    \label{thm:breuer_hall_indecomposable}
    \lean{Matrix.breuerHallMap_isIndecomposablePositiveMap,
        Matrix.breuerHallMap_not_isDecomposablePositiveMap}
    \leanok
    \uses{def:breuer_hall_map, def:decomposable_positive_map,
        thm:breuer_hall_positive, thm:indecomposable_of_ppt_witness}
    Suppose $D>2$ and $U$ is an antisymmetric unitary on $\C^{D}$,
    $U^{\mathsf T}=-U$ and $U^{\dagger}U=\Id$. Then the Breuer--Hall map
    $T_{\mathrm{BH}}$ is an indecomposable positive map.
\end{theorem}

\begin{proof}
    \leanok
    \uses{def:breuer_hall_map, thm:breuer_hall_positive,
        thm:indecomposable_of_ppt_witness}
    Positivity is Theorem~\ref{thm:breuer_hall_positive}. For
    indecomposability, write $F$ for the swap operator on
    $\C^{D}\otimes\C^{D}$ and $\ket{\Psi}$ for the $U$-twisted maximally
    entangled vector, $\Psi(i,k)=U(i,k)$, with unnormalized projector
    $P_{0}=\ket{\Psi}\bra{\Psi}$. The witness is the bipartite state
    \begin{align}
        \rho & =(\Id+F)+P_{0},
        \notag
    \end{align}
    a sum of positive semidefinite terms ($\Id+F$ is twice the projector onto
    the permutation-symmetric subspace), hence positive semidefinite. Its
    first-factor partial transpose is
    \begin{align}
        \rho^{T_{1}} & =P_{1}+(U^{\dagger}\otimes\Id)(\Id+F)(U\otimes\Id),
        \notag
    \end{align}
    where $P_{1}$ is the untwisted maximally entangled projector: the partial
    transpose of $\Id+F$ is $\Id+P_{1}$, and antisymmetry together with
    unitarity of $U$ gives
    $P_{0}^{T_{1}}=(U^{\dagger}\otimes\Id)(\Id+F)(U\otimes\Id)-\Id$, so the
    identity summands cancel and $\rho^{T_{1}}$ is again a sum of positive
    semidefinite terms. Hence $\rho$ has the PPT property. The carrying
    identity, valid for every bipartite $\sigma$ and proved by expanding all
    tensor indices and contracting against $U^{\dagger}U=\Id$, is
    \begin{align}
        \tr\!\big[(T_{\mathrm{BH}}\otimes\id)(\sigma)\,P_{0}\big]
         & =\tr(\sigma)-\tr(\sigma P_{0})-\tr(F\sigma).
        \notag
    \end{align}
    At $\sigma=\rho$ the three traces evaluate to $\tr(\rho)=D^{2}+2D$,
    $\tr(\rho P_{0})=D^{2}$, and $\tr(F\rho)=D^{2}$ --- the last two because
    antisymmetry gives $F\ket{\Psi}=-\ket{\Psi}$ --- and therefore
    \begin{align}
        \tr\!\big[(T_{\mathrm{BH}}\otimes\id)(\rho)\,P_{0}\big]
         & =D(2-D)<0.
        \notag
    \end{align}
    So the quadratic form of $(T_{\mathrm{BH}}\otimes\id)(\rho)$ at
    $\ket{\Psi}$ is negative and $(T_{\mathrm{BH}}\otimes\id)(\rho)$ is not
    positive semidefinite. By
    Theorem~\ref{thm:indecomposable_of_ppt_witness}, $T_{\mathrm{BH}}$ is not
    decomposable.
\end{proof}
