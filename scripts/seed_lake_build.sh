#!/usr/bin/env bash
# APFS-clone an existing QICLean .lake into a fresh worktree.
set -euo pipefail
...
validate_packages() {
  ...
  local metadata_link
  local reported_root
  PACKAGE_LIST_FILE="$(mktemp "${TMPDIR:-/tmp}/qiclean-lake-packages.XXXXXX")" ||
...
REUSE_QICLEAN_BUILD="false"
if test "$SOURCE_REV" = "$TARGET_REV"; then
  REUSE_QICLEAN_BUILD="true"
fi
...
if test "$DRY_RUN" = "true"; then
  ...
  if test "$REUSE_QICLEAN_BUILD" = "true"; then
    echo "seed-lake-build: QICLean build artifacts will be reused"
  else
    echo "seed-lake-build: QICLean build artifacts will be omitted (different revisions)"
  fi
  exit 0
fi
...
if test "$REUSE_QICLEAN_BUILD" = "true"; then
  ...
  echo "seed-lake-build: omitted QICLean build artifacts from different source revision"
fi
