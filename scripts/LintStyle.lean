/-
Copyright (c) 2026 The QICLean Contributors. All rights reserved.
...
/-!
# `lake exe lint_style` for QICLean

Invokes Mathlib's `Mathlib.Linter.TextBased.lintModules` on every `.lean` file under
`QICLean/` and on the top-level `QICLean.lean` import surface. Runs the text-based style
...
def main (args : List String) : IO UInt32 := do
  ...
  let mut modules ← collectLeanModules "QICLean"
  if ← (FilePath.mk "QICLean.lean").pathExists then
    modules := modules.push `QICLean
  if modules.isEmpty then
    throw <| IO.userError "lint_style: no `.lean` files found under QICLean/ or at QICLean.lean"
